...
export class LearningPageComponent implements OnInit {
  ...
  loadNextTopic(): void {
    if (this.currentTopic) {
      this.topicService.getNextTopic(this.currentTopic.id).subscribe(nextTopic => {
        if (nextTopic) {
          const nextIndex = this.progress.current;
          this.router.navigate(['/learn'], { queryParams: { topic: nextIndex } });
        }
      });
    }
  }
  ...
  loadPreviousTopic(): void {
    if (this.currentTopic) {
      this.topicService.getPreviousTopic(this.currentTopic.id).subscribe(prevTopic => {
        if (prevTopic) {
          const prevIndex = this.progress.current - 2;
          this.router.navigate(['/learn'], { queryParams: { topic: prevIndex } });
        }
      });
    }
  }
  ...
  updateNavigationState(): void {
    if (this.currentTopic) {
      this.topicService.getPreviousTopic(this.currentTopic.id).subscribe(prev => {
